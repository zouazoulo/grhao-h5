define('framOther',['common','vue'],function(require, exports, module){
	...
	pub.firmId =  sessionStorage.getItem("firmId")
	if (pub.logined) {
	...
	pub.Vue = new Vue({
		...
		methods: {
		   ...
		   Select:function(index){
		   		this.defaultSelect = index;//this指向app
		   		console.log(this.num)
		   },
		   giveThings:function(){
		   		var target = event.target;
		   ...
		   invitationFriend:function(){ //邀请好友弹窗
	      	...
	      	defaultSelected:function(index){
	      		var a ={
	      			id:this.storeSelection[index].id,
	      			address:this.storeSelection[index].address,
	      			firmName:this.storeSelection[index].firmName
	      		}
	      		sessionStorage.setItem('firmId',JSON.stringify(a))
...
//	};
	...
	farmExchange.apiHandle = {
		init:function(){
			...
			pub.Vue.addressInfo = JSON.parse(pub.firmId)
			console.log(pub.Vue.fruit)
		...
		exchange_fruit:{
			init:function(){
				...
				common.ajaxPost($.extend({},{
					...
					firmId:JSON.parse(pub.firmId).id
...
//			pub.Vue.storeSelection =data.data.hasPage.objects
			...
			if(data.data.hasPage.objects != 0){
				storeSelection.eventHandle.storeSelection(data.data.hasPage);
			}
			
		}
	}
	storeSelection.eventHandle={
			storeSelection:function(v){
				var o ={};
					if(pub.Vue.pageNo == 1){
						o={
							pageNo:1,//动态的当前页
							isLast:v.isLast,//是否是最后一页
							objects:v.objects,//动态的消息列表
						}
					}else{
						o = $.extend({},pub.Vue.storeSelection, {
							pageNo:pub.Vue.pageNo,//动态的当前页
							isLast:v.isLast,//是否是最后一页
							objects:pub.Vue.storeSelection.objects.concat(v.objects),//动态的消息列表
						});
					}
					pub.Vue.storeSelection = o;
					console.log(o)
