...
*/ 
define('orderSettlement',['common','goshopCar','LArea'],function(require, exports, module){
	...
	PLAIN.apiHandle = {
		...
		shop_cart_submit : {
			...
			apiData : function( d ){
				...
				PLAIN.apiHandle.shop_cart_submit.goodsPriceInfo( orderInfo ); // 商品价格信息

				!!calendar && !!time && PLAIN.apiHandle.shop_cart_submit.dispatchTime( calendar, time ); // 派送时间
				!!couponInfolist && pub.orderType == "1" && PLAIN.apiHandle.shop_cart_submit.couponList( couponInfolist ); // 优惠券列表
			},
			// 优惠券列表
			couponList : function( couponInfolist ){
				...
				nodeBox.show().find('.set_coupon_box1_bottom-inner').html( html );
			...
			goodsPriceInfo : function( orderInfo ){
				...
				//优惠券金额
				(function(){
					pub.orderType == "3" ? listNode.eq(2).hide() : listNode.eq(2).show().find(".float_right").html( orderInfo.couponMoney > 0 ? "-￥" + orderInfo.couponMoney : "￥0" );
				}());
				// 优惠券优惠价格信息
				(function(){
					if( 0 < orderInfo.couponStrategy && orderInfo.couponStrategy < 5  ){
						...
						listNode.eq(3).show().find(".float_left").html( couponInfo.text ).next().show().html(function(){
							...
							if( sign == '-￥' ) 
								return sign + orderInfo[couponInfo.key];
							return orderInfo[couponInfo.key] + sign;
						});
					}else{
						listNode.eq(3).hide();
					}
	...
	pub.eventHandle = {
		init : function(){
			...
			common.jumpLinkSpecial(".header_left",function(){
				...
				pub.dom ? window.location.replace('../index.html') : window.history.back();
			});
			// 优惠券说明 
