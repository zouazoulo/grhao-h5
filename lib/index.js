...
define('index', ['common'], function(require, exports, module){
    ...
    pub.apiHandle.weixin_binding = {
    	init : function(){
    		common.ajaxPost({
    			method : 'weixin_binding',
    			openId : pub.openId,
    			userId : common.user_datafn().cuserInfoid
    		},function( d ){
    			d.statusCode == '100000' && common.accountRelatived.setItem('accountRelatived');
    		});
    	},
    };
	//微信自动登录
	pub.apiHandle.weixin_auto_login = {
		init : function (){
			common.ajaxPost({
	            method: 'weixin_login',
	            openId : pub.openId,
	            ...
	            if( d.statusCode == '100000'){
					pub.apiHandle.weixin_auto_login.apiData(d);		                
	...
	pub.eventHandle = {
 	...
 	pub.init = function(){
		...
		pub.watmId = common.getUrlParam("watmId");//获取机器编码id
 		...
 		*/
 		...
 		pub.isWeiXin && (function(){
 			!pub.openId ? (function(){
 				pub.weixinCode ? pub.get_weixin_code() : common.jumpLinkPlain('https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxf9dd00234aa6e921&redirect_uri=' + pub.domain + '/test0319/&response_type=code&scope=snsapi_userinfo&state=grhao&connect_redirect=1#wechat_redirect');
 			}()) : (function(){
