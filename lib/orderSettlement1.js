	...
	PLAIN.apiHandle = {
		...
		shop_cart_submit : {
			...
			goodsPriceInfo : function( orderInfo , dataList){
				...
				pub.storeInfo.type == 5 ? listNode.eq(1).find(".float_left").html("配送费") : listNode.eq(1).find(".float_left").html("运费");
				pub.pickUpMethod == 2 ? curNode.next().show().find(".float_right").html( "￥" + orderInfo.postCost ) : curNode.next().hide(); // 运费
				//是否是首单
		...
		storeInfo : {
			init : function(){
				...
				if( pub.storeInfo.type == 5 ){
					$('.set_charge_contact_right').find('.take_own').prev().html('售货机自提');
