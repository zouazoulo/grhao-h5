...
*/
define('payment',['common','vue'],function(require, exports, module){
	...
	PW.czkRechange = function(){
		pub.recahngeCard = $("#rechange_card").val();
		pub.exchangeCode = $("#rechange_exchangeCode").val();
		var reg_card = /^([0-9A-Za-z]{12}$/
		var reg_exchange = /^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6}$/
		if( !reg_card.test( pub.recahngeCard ) ) {
            common.prompt( "请输入正确的充值卡号" ); return;
        }
        // 充值
       	if( !reg_exchange.test( pub.exchangeCode ) ) {
