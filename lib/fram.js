define('fram',['common','farmCanves','vue'],function(require, exports, module){
	...
	pub.Vue = new Vue({
		...
		methods: {
			...
			goExchangeRecord:function(){
				window.location.href = "../html/farmExchangeRecord.html"
			},
			//关闭遮罩
			...
			getSignIn: function (item,type) {
				if (type == 1) {
					if (item.sign == 2) {
						...
						var data = item.signDate,
							id = item.id;
						pub.apiHandle.farm_sign_show.init(data,id)
					}else{
			...
			getWatering: function (data) {  
	      	...
	      	//翻转
      	 	turn_back:function(){
			    ...
			    var target = event.target;
			    if(target.parentNode.parentNode.parentNode.classList.contains('flip')){
			    	eleFront= target.parentNode.parentNode.parentNode
			    	eleBack = eleFront.previousElementSibling
			    }else{
			    	eleFront = target.parentNode.parentNode;
			    	eleBack =eleFront.nextElementSibling;
			    }
			...
			//邀请送礼
			gift:function(num){
				var b , c , d ,child,child1,child2,urlImg = "../img/farmImg/gift5.png";
				var target = event.target;
				var a = target.nextElementSibling.nextElementSibling.firstChild;
				d = a.firstChild
				child = d.firstChild;
				child1 = d.nextElementSibling.firstChild;
				child2  = d.nextElementSibling.nextElementSibling.firstChild; 
				c = a.style.width;
				b = parseInt(c)+num*76+'px';
				a.style.width = b;
				if(parseInt(a.style.width) >= 265){
					child.src = urlImg
					if(parseInt(a.style.width) >= 374 ){
						child1.src = urlImg
						if(parseInt(a.style.width) >= 670){
							child2.src = urlImg
						}
					}
				}
			...
			clickGiveFriends:function(event){		
				...
				window.location.href = "../html/giveFriends.html"
			},
	...
	*/
	...
	pub.apiHandle = {
		...
		//显示签到礼包的详情
		farm_sign_show:{
			init:function(signDate,id){
				common.ajaxPost($.extend({},{
					method: 'farm_sign_show',
					signDate: signDate,
					id:id,
				},pub.userBasicParam), function(d) {
					d.statusCode == "100000" ? pub.apiHandle.farm_sign_show.apiData(d) : common.prompt(d.statusStr);
				})
			},
			apiData:function(d){
				var v = d.data;
