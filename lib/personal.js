...
define('personal',['common','exif'],function(require, exports, module){
	...
	pub.apiHandle = {
		init : function(){
			...
			if( pub.logined ){
				...
				if (common.isWeiXin() && pub.logined && !common.user_datafn().mobile) {
					$(".main_bottom .bind_item").css("display","block");
				}
			}else{
	...
	pub.eventHandle.init = function(){
		...
		$('.main_top_login').click(function(){
			...
			if( common.isWeiXin() && !commom.opendId.getKey() ){
				common.jumpLinkPlain('https://open.weixin.qq.com/connect/oauth2/authorize?appid='+pub.appid+'&redirect_uri='+pub.domain+'/html/login.html&response_type=code&scope=snsapi_userinfo&state=grhao&connect_redirect=1#wechat_redirect');
			}else{
				common.jumpLinkPlain('login.html');
			}
		});
		...
		$('.zs_personal').click(function(){
			...
			var bind_mobile = $(this).is(".bind_mobile");
			var isChat = $(this).is(".zs_onlineChat");
			if( pub.logined ){
				if (isChat) {
...
/********************* 用户信息修改模块 *****************/
	...
	USER_INFO_REPAIRED.eventHandle = {
		init : function(){
			...
			$('.message_phoneNumber').click(function(){
				//点击先判断是否有手机号
				//有手机号修改没有手机号添加
				var isMobile = $("#message_phoneNumber").val();
				if (isMobile) {
					...
					USER_INFO_REPAIRED.switchInput('完善手机号','.zs_address_box','.zs_phone_box_new');
				}
			});
			...
			$('.header_left').click(function(){
				...
				console.log($(".zs_address_box").is(":hidden"))
				if ($(".zs_address_box").is(":hidden")) {
					if($('.zs_phone_box').is(':hidden')){
						USER_INFO_REPAIRED.switchInput('修改信息','.zs_phone_box_new','.zs_address_box');
					}else{
			...
			$('#get_verify_code').on('click',function(){
				USER_INFO_REPAIRED.time = 59;
				USER_INFO_REPAIRED.phone = $("#login_phoneNumber2").val();
				if(!common.PHONE_NUMBER_REG.test( USER_INFO_REPAIRED.phone )){
					common.prompt('手机号输入错误');return;
				}
				$("#verify_code").removeAttr("disabled");
				$("#get_verify_code").css("display",'none');
				$("#time").css({"display":"block","color":"#f76a10","background":"none"}).html('(60s后重试)');
				USER_INFO_REPAIRED.apiHandle.send_new_mobile.init();
				USER_INFO_REPAIRED.countDown(); // 验证码倒计时
			});
			...
			$(".login_btn").on("click",function(){
				...
				USER_INFO_REPAIRED.verify_code = $("#verify_code").val();
				USER_INFO_REPAIRED.phone = $("#login_phoneNumber2").val();
				
				if( USER_INFO_REPAIRED.phone == "" ){
					common.prompt('请输入手机号'); return;
				}
				...
				USER_INFO_REPAIRED.apiHandle.binding_new_mobile.init();
			});
	...
	// 验证码接口数据处理
	USER_INFO_REPAIRED.apiHandle.send_new_mobile = {
		init : function(){
			common.ajaxPost($.extend({},pub.userBasicParam,{
	...
	USER_INFO_REPAIRED.apiHandle.binding_new_mobile = {
		init : function(){
			common.ajaxPost($.extend({},pub.userBasicParam,{
	...
	USER_INFO_REPAIRED.countDown = function(){
		pub.timer_id = setInterval(function(){
			if ( USER_INFO_REPAIRED.time == 0 ) {
				$("#phone_time,#time").css("display","none");
				$("#phone_verify_code1,#get_verify_code").css("display","block").html('重新获取')
				clearInterval( pub.timer_id );
				USER_INFO_REPAIRED.time = 59;
			}else{
				$("#phone_time,#time").html("( " + USER_INFO_REPAIRED.time + " s后可重试 )");
			}
...
/****************************  修改密码模块  ************************/
	...
	BINDUSER.eventHandle = {
		init : function(){
	       	...
	       	$('.zs_get_verify_code').on('click',function(){
				...
				pub.countDown();// 倒计时开始
				BINDUSER.apiHandle.send_sms.init(); // 请求验证码
			});
			//绑定
			$('.login_btn').click(function(){
				...
				BINDUSER.smsCode = $('#verify_code').val();
				BINDUSER.apiHandle.binding_mobile.init(); // 绑定原有手机号
			});
		},
	};
	...
	BINDUSER.apiHandle = {
		...
		send_sms : {
			init:function(){
				common.ajaxPost($.extend({},pub.userBasicParam,{
					method : 'send_binding_mobile',
					mobile : pub.phoneNum
				}),function( d ){
					d.statusCode == '100000' ? 
						BINDUSER.apiHandle.send_sms.apiData(d) :
						common.prompt( d.statusStr )
				});
			},
			apiData:function(d){
		...
		binding_mobile : {
			init:function(){
				common.ajaxPost($.extend({},pub.userBasicParam,{
					method : 'binding_mobile',
					mobile : pub.phoneNum,
					smsCode : BINDUSER.smsCode
				}),function( d ){
					d.statusCode == '100000' ? 
						BINDUSER.apiHandle.binding_mobile.apiData(d) :
						common.prompt( d.statusStr )
				});
			},
			apiData:function(d){
	...
	// 绑定原有账号
	BINDUSER.init = function(){
		BINDUSER.eventHandle.init();
		BINDUSER.apiHandle.init();
	};	
	// 换肤
	...
	COUPON_LIST.eventHandle = {
		init : function(){
	    	...
	    	$('.lodemore').on('click',function(){				
				if ( $(this).html() != '没有更多数据了' ) {
					pub.PAGE_INDEX++;
					COUPON_LIST.apiHandle.couponInfo_manager.init();
				}else{
					$(this).off('click');
				}				
			});
    		...
    		var bool = true;
    		$('.cuopon_management_top_left').click(function(){
    			USER_INFO_REPAIRED.switchInput.call(COUPON_LIST,'优惠券','.zs-couponManager-box','.zs-coupon-box','在线获取优惠券');
    			bool && (function(){
    		...
    		$('.header_left').click(function(){
				...
				$('.zs-coupon-box').is(':visible') ? USER_INFO_REPAIRED.switchInput.call(COUPON_LIST,'优惠券管理','.zs-coupon-box','.zs-couponManager-box') : window.history.back();
			});
...
/************************* 新改版失效优惠券模块2018-06-22 ***************************/
	...
	COUPON_LIST_INVALID.eventHandle = {
		init : function(){
	    	...
	    	$('.lodemore').on('click',function(){				
				if ( $(this).html() != '没有更多数据了' ) {
					pub.PAGE_INDEX++;
					COUPON_LIST_INVALID.apiHandle.couponInfo_manager.init();
				}else{
					$(this).off('click');
				}				
			});
    		...
    		$('.header_left').click(function(){
				$('.zs-coupon-box').is(':visible') ? USER_INFO_REPAIRED.switchInput.call(COUPON_LIST_INVALID,'优惠券管理','.zs-coupon-box','.zs-couponManager-box') : window.history.back();
			});

			window.onpopstate = function(){
				USER_INFO_REPAIRED.switchInput.call(COUPON_LIST_INVALID,'优惠券管理','.zs-coupon-box','.zs-couponManager-box');
				$('.pop').hide();
			}
		}
	};
	...
	pub.init = function(){
		...
		switch( +pub.muduleId ){
			...
			case 7 : BINDUSER.init(); break; //绑定原有账号
			case 21 : COUPON_LIST.init();break;//新增优惠劵列表页面
			case 22 : COUPON_LIST_INVALID.init();break;//失效劵列表页面
		};
