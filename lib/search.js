			...
			$('#store-list-data-box').on('click','.list',function(e){
				...
				if( $(e.target).is('.isCurNode') ){ // 是图标节点，并且授权成功
					...
					common.dialog.init({
	                    ...
	                    common.watmId.removeItem();
	                    STORE.storeInfo.id != pub.storeInfo.id && common.good.removeItem();
			...
			if(common.isWeiXin()){
				...
				wx.getLocation({
					...
					success: function (res) {
						pub.getLng = res.longitude;
				     	pub.getLat = res.latitude;
				     	MAP.coords.user = [+res.longitude , +res.latitude];
				     	STORE.apiHandle.choose_firm_list.init();
		...
		change_app_theme : {
			init:function(){
				if (common.huanfu.getItem() && common.huanfu.getItem() != 1) {
					$(".store-box,.header_contain").addClass("skin"+sessionStorage.getItem("huanfu"))
				}
			}
		}
	};
