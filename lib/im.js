 ...
 * */
	...
	var msgObj = {
		 ...
		 * */
		updateMsg:function(){
			var msgAll = this.getMsg();
			var nowTimestamp = Date.parse(new Date());
			function removeItem (msgAll){
				if (msgAll.length != 0) {
					//删除超过三天的数据
					if ((nowTimestamp - msgAll[0].createTime) > this.oneDayMillisecond *3 ) {
						removeItem(msgAll.slice(1))
					}
				}
			}
		},
		...
		onMessage:function (msg) {
		    ...
		    if (type == "chat" && elems.length > 0) {
				...
				if (o) {
					...
					mo.content = o.content;

                    mo.msgType = o.msgType;
          	...
          	play_voice:function(e,item,index){
          		...
          		function creatVoice(item,index){
          			if (pub.Vue.Voice) {
	          			...
	          			audio = $("<audio preload='auto' hidden='true' data-index='"+index+"'><source src='"+item.content+"' type='audio/mpeg'></audio>")[0]
	          			pub.Vue.Voice = audio;
				...
				function getNoread(index){
					var msgs = pub.Vue.msgBody,
          			...
          			for (var i = index; i < msgL ; i++) {
          				if (msgs[i].type == "voice" && msgs[i].read == '0') {
          					return {
          						item:msgs[i],
          						index:i,
          					}
 		...
 		init:function(){
 			...
 			pub.dfd.done(function(){
 				pub.dfdDefault.done(function(){
                    ...
                    msgObj.init(key);
                    console.log(msgObj.msgDates)
                    pub.Vue.msgBody = msgObj.msgDates;
 				})
 			})
 		},
 			...
 			common.ajaxPost({
	            ...
	            if( d.statusCode == '100000' ){
	            	if(d.data == 2 || d.data == 1){
	            		...
	            		configData.userImg = common.user_datafn().faceImg;
	            	}else{
	            		config.userId = ''
	            	}
	            }else{
