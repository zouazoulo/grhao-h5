...
define('common',['md5','sha1','jquery','swiper','weixinSDK','module'],function(require, exports, module){
	...
	$.extend(ZS,{
		...
		APIFARM : "http://61.164.113.168:8090/grh_api/server/farm.do", // 测试
	...
	*/
	...
	$.extend(ZS,{
		...
		jumpLinkPlain : function( url ){
			...
			if (url.indexOf("?") > 1) {
				window.location.href = url+"&v=1011";
			}else{
				window.location.href = url+"?v=1011";
			}
			return true; 
		...
		createPopup	: function(opt) {
	        ...
	        case 11:
	            $el.html('<span class="close"></span><div class="msg signList">'+msg+'</div><p class="btns signBtn"><a href="javascript:;" class="btn btn_1">' + opt.okText + '</a></p>');
	            btnClose = 'span.close';
	            btnConfirm = 'p.btns';
	            break;
	        }
