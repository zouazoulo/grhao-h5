...
define('common',['md5','sha1','jquery','swiper','weixinSDK','module'],function(require, exports, module){
	...
	require('module');
	$.extend(ZS,{
		API : "http://api.grhao.com/server/api.do", // 正式
		APIFARM : "http://api.grhao.com/server/farm.do", // 正式
		...
		WXDOMAIN : "http://weixin.grhao.com", //正式
		//appid 测试掌烁科技wxe92e098badc60fab正式果然好商城wxf9dd00234aa6e921
		WXAPPID : "wxf9dd00234aa6e921", //正式
	});
	//换肤延时对象
	...
	*/
	...
	$.extend(ZS,{
		...
		jumpLinkPlain : function( url ){
			...
			if (url.indexOf("?") > 1) {
				window.location.href = url+"&v=20181023";
			}else{
				window.location.href = url+"?v=20181023";
			}
			return true; 
	    ...
	    createGamePopup : function(opt) {
	        ...
	        case 2:
	            ...
	            break;  
	        case 3:
	            $el.html('<span class="close"></span><h3 class="title">恭喜获得</h3><div class="msg">'+msg+'</div>');
	            btnClose = 'span.close';
	            break;  
	        }
	         setTimeout(function() {
	...
	ZS.weixin = {
				...
				success : function( d ){
					if( d.statusCode == '100200' ){
						...
						wx.config({
						    ...
						    jsApiList:["onMenuShareTimeline","onMenuShareAppMessage","onMenuShareQQ","onMenuShareQZone","getLocation"] // 必填，需要使用的JS接口列表，所有JS接口列表见附录2
						});
		...
		share : function( data, done, cancle ){
		    ...
		    var dataConfig = {
	    	    ...
	    	    //link: encodeURIComponent('http://wx.grhao.cn/lifei/index.html?farmerId=17'),
	    	    link: data.link || document.location.origin,
	    	    imgUrl: data.imgUrl,
	...
	/*
	 ...
	 输入位 Y-M-D h:m:s
	 * */
	ZS.timestampToTime = function(timestamp) {
